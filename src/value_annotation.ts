...
export class WaveformValueAnnotationProvider {
  ...
  private decorationTypesMap = new Map<
    string,
  ...
  async listenToMarkerSetEventEvent(): Promise<vscode.Disposable | undefined> {
    ...
    const waveformViewer = vscode.extensions.getExtension(
      "lramseyer.vaporview",
    );
    if (!waveformViewer) {
      return undefined;
    ...
    if (api && api.onDidSetMarker) {
      ...
      const disposable = api.onDidSetMarker((data: any) => {
        ...
        if (
          data.uri.fsPath !==
          this.hierarchyTreeProvider.getActiveDesign()?.getActiveWaveform()
            ?.resourceUri.fsPath
        ) {
          return;
        }
  ...
  public async handleChangeVisibleTextEditors() {
    ...
    if (
      this.targetFile &&
      this.decorationTypesMap.size !== 0 &&
      this.rangesMap.size !== 0
    ) {
  ...
  public async handleChangeTextDocument(e: vscode.TextDocumentChangeEvent) {
    ...
    if (this.targetFile !== e.document.uri.fsPath) {
      return;
    }
    this.debounceUpdateDecorations();
  }

  public async handleActiveInstanceChanges(
    e: void | NetlistItem | null | undefined,
  ) {
    if (!e) {
      return;
    }
    ...
    if (this.activeInstance && this.activeInstance === e) {
      return;
    }
  ...
  public async handleActiveWaveformChanges(
    e: void | vscode.TreeItem | null | undefined,
  ) {
    this.debounceUpdateDecorations();
  }
  ...
  public async updateDecorationsForEditor(editor: vscode.TextEditor) {
    ...
    const nodes = await this.parser.parseAndCollectIdentifiersInModule(
      document,
      activeModule,
    );
    ...
    const instancePaths =
      await activeInstance.getAllChildVarsNames(activeDesign);

    // Get waveform values from waveform viewer
    const waveformValues = await vscode.commands.executeCommand<
      { instancePath: string; value: string }[]
    >("waveformViewer.getValuesAtTime", {
      uri: activeWaveform.resourceUri.toString(),
    ...
    await this.hierarchyTreeProvider.updateWaveformValuesDescriptions(
      activeInstance,
      waveformValueMap,
    );
    ...
    for (const node of nodes) {
      if (!waveformValueMap.has(node.text)) {
        continue;
      }
