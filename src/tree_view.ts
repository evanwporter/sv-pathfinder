...
const taskIcon = new vscode.ThemeIcon(
  "debug-stackframe",
  new vscode.ThemeColor("charts.blue"),
);
const funcIcon = new vscode.ThemeIcon(
  "symbol-module",
  new vscode.ThemeColor("charts.blue"),
);
const beginIcon = new vscode.ThemeIcon(
  "debug-start",
  new vscode.ThemeColor("charts.blue"),
);
const forkIcon = new vscode.ThemeIcon(
  "repo-forked",
  new vscode.ThemeColor("charts.blue"),
);
const structIcon = new vscode.ThemeIcon(
  "symbol-structure",
  new vscode.ThemeColor("charts.blue"),
);
const unionIcon = new vscode.ThemeIcon(
  "surround-with",
  new vscode.ThemeColor("charts.blue"),
);
const classIcon = new vscode.ThemeIcon(
  "symbol-misc",
  new vscode.ThemeColor("charts.blue"),
);
const interfaceIcon = new vscode.ThemeIcon(
  "debug-disconnect",
  new vscode.ThemeColor("charts.purple"),
);
const packageIcon = new vscode.ThemeIcon(
  "package",
  new vscode.ThemeColor("charts.purple"),
);
const scopeIcon = new vscode.ThemeIcon(
  "symbol-module",
  new vscode.ThemeColor("charts.purple"),
);
const moduleDefIcon = new vscode.ThemeIcon("symbol-enum");
const modportIcon = new vscode.ThemeIcon(
  "plug",
  new vscode.ThemeColor("charts.orange"),
);
const clockIcon = new vscode.ThemeIcon(
  "watch",
  new vscode.ThemeColor("charts.blue"),
);
...
// Variables
const regIcon = new vscode.ThemeIcon(
  "symbol-array",
  new vscode.ThemeColor("charts.green"),
);
const wireIcon = new vscode.ThemeIcon(
  "symbol-interface",
  new vscode.ThemeColor("charts.pink"),
);
const intIcon = new vscode.ThemeIcon(
  "symbol-number",
  new vscode.ThemeColor("charts.green"),
);
const paramIcon = new vscode.ThemeIcon(
  "settings",
  new vscode.ThemeColor("charts.green"),
);
const realIcon = new vscode.ThemeIcon(
  "pulse",
  new vscode.ThemeColor("charts.orange"),
);
const defaultIcon = new vscode.ThemeIcon(
  "file-binary",
  new vscode.ThemeColor("charts.green"),
);
const stringIcon = new vscode.ThemeIcon(
  "symbol-key",
  new vscode.ThemeColor("charts.yellow"),
);
const portIcon = new vscode.ThemeIcon(
  "plug",
  new vscode.ThemeColor("charts.green"),
);
const timeIcon = new vscode.ThemeIcon(
  "watch",
  new vscode.ThemeColor("charts.green"),
);
const variableIcon = new vscode.ThemeIcon(
  "symbol-variable",
  new vscode.ThemeColor("charts.green"),
);
...
export function createVar(
  ...
  const variable = new NetlistItem(
    fullName,
    type,
    width,
    file,
  ...
  if (
    ...
    if (width > 1) {
      variable.iconPath = regIcon;
    } else {
      variable.iconPath = wireIcon;
    }
  }
...
export class NetlistItem extends vscode.TreeItem {
  ...
  constructor(
    ...
    if (this.contextValue === "moduleDefItem") {
      this.modulePath = "";
    }
  ...
  async findChild(
    ...
    if (childItem) {
      ...
      if (
        childItem.type === "scopearray" ||
        childItem.type === "instancearray"
      ) {
  ...
  async getAllChildVarsNames(design: DesignItem): Promise<string[]> {
    let names: string[] = [];
    if (this.contextValue !== "scopeItem") {
      return names;
    }
    if (this.children.length === 0) {
      await design.getChildrenExternal(this);
...
export abstract class DesignItem extends vscode.TreeItem {
  ...
  public abstract getChildrenExternal(
    element: NetlistItem | undefined,
  ): Promise<NetlistItem[]>;
  public abstract getDriversAndLoadsExternal(
    element: NetlistItem,
  ): Promise<void>;
  public abstract getModuleInstancesExternal(
    element: NetlistItem | undefined,
  ): Promise<NetlistItem[]>;
  public abstract getDefinitionFileLocation(
    element: NetlistItem,
  ...
  public async setActiveInstance(element: NetlistItem): Promise<boolean> {
    ...
    if (element.contextValue === "scopeItem") {
      ...
      element.contextValue === "loadItem" ||
      element.contextValue === "driverItem"
    ) {
      // find tree item using modulePath
      const scope = await this.findTreeItem(element.modulePath);
      if (!scope) {
        console.log(
          "[setActiveInstance] Cannot find in hierarchy tree: " +
            element.fullName,
        ); // Should not happen
      } else {
        instance = scope;
  ...
  public getActiveScope(): string {
    ...
    if (activeInstance.contextValue === "scopeItem") {
      const label =
        typeof activeInstance.label === "string" ? activeInstance.label : "";
  ...
  public addWaveform(uri: vscode.Uri): WaveformItem | undefined {
    let index = this.waveforms.findIndex(
      (waveform) => waveform.resourceUri.fsPath === uri.fsPath,
    );
    if (index < 0) {
      const waveform = new WaveformItem(
        uri,
        this,
        vscode.TreeItemCollapsibleState.None,
      );
      ...
      if (this.waveforms.length === 1) {
        // Set active if it's the only waveform
  ...
  public setActiveWaveform(waveform: WaveformItem | undefined) {
    if (this.activeWaveform === waveform) {
      return;
    }
  ...
  public async findTreeItem(
    ...
    const element = this.treeData.find(
      (element) => element.name === fullName.split(".")[0],
    );
    if (!element) {
      return undefined;
    }
    return await element.findChild(
...
function parseSlangType(typeDecl: string): {
  ...
  const re =
    /^\s*(input|output|inout)?\s*(\w+)\s*(\[\s*\d+\s*:\s*\d+\s*\])?\s*$/i;
...
class SlangDesignItem extends DesignItem {
  ...
  private async loadTopModules() {
    for (const top of this.unit!) {
      if (top.kind === slang.SlangKind.Instance) {
        // not handle packages yet
    ...
    for (const child of children) {
      ...
      switch (child.kind) {
        case slang.SlangKind.Instance:
          ...
          if (element.type === "instancearray") {
            fullName = element.fullName + child.instName;
          }
          scope = createScope(
            fullName,
            "module",
        ...
        case slang.SlangKind.InstanceArray:
          ...
          const [subSubScopes, subVariables] = this.collectChildren(
            scope,
            child.children,
          );
        ...
        case slang.SlangKind.Logic:
          ...
          if (child.kind === slang.SlangKind.Port) {
            type = "port";
          }
          const v = createVar(
            fullName,
        ...
        case slang.SlangKind.Scope:
          ...
          const [subSubScopes2, subVariables2] = this.collectChildren(
            scope,
            child.children,
          );
        ...
        case slang.SlangKind.ScopeArray:
          ...
          scope.description = child.declName;
          const [subScopes3, subVariables3] = this.collectChildren(
            scope,
            child.children,
          );
...
export class OpenedDesignsTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  private _onDidChangeActiveWaveformForActiveDesign: vscode.EventEmitter<
    vscode.TreeItem | undefined | null | void
  > = new vscode.EventEmitter<vscode.TreeItem | undefined | null | void>();
  public readonly onDidChangeActiveWaveformForActiveDesign: vscode.Event<
    vscode.TreeItem | undefined | null | void
  > = this._onDidChangeActiveWaveformForActiveDesign.event;
  ...
  public async openWaveform(
    element: DesignItem,
  ): Promise<WaveformItem | undefined> {
    const options: vscode.OpenDialogOptions = {
      canSelectFiles: true,
  ...
  private setActiveWaveformForDesign(
    design: DesignItem,
    waveform: WaveformItem | undefined,
  ) {
  ...
  public async closeWaveform(element: WaveformItem) {
    ...
    if (design instanceof DesignItem) {
      // always true
  ...
  public handleCheckWaveformItem(element: vscode.TreeItem) {
    ...
    if (design instanceof DesignItem) {
      ...
      for (const waveform of design.getWaveforms()) {
        if (waveform instanceof WaveformItem) {
          // always true
          if (waveform !== element) {
...
function replaceFilePathIfNeeded(filePath: string, isExample: boolean = false) {
  ...
  const from = vscode.workspace
    .getConfiguration("sv-pathfinder")
    .get("remotePathPrefix", "");
  const to = vscode.workspace
    .getConfiguration("sv-pathfinder")
    .get("localPathPrefix", "");
  if (!from || !to || from === to) {
...
async function showTextDocumentLocation(
  filePath: string,
  lineNumber: number,
  columnNumber: number,
...
export class HierarchyTreeProvider implements vscode.TreeDataProvider<NetlistItem> {
  ...
  constructor(
    ...
    this.activeScopeStatusBarItem = vscode.window.createStatusBarItem(
  ...
  private _onDidChangeTreeData: vscode.EventEmitter<
    NetlistItem | undefined | null | void
  > = new vscode.EventEmitter<NetlistItem | undefined | null | void>();
  public readonly onDidChangeTreeData: vscode.Event<
    NetlistItem | undefined | null | void
  > = this._onDidChangeTreeData.event;

  private _onDidChangeActiveInstance: vscode.EventEmitter<
    NetlistItem | undefined | null | void
  > = new vscode.EventEmitter<NetlistItem | undefined | null | void>();
  public readonly onDidChangeActiveInstance: vscode.Event<
    NetlistItem | undefined | null | void
  > = this._onDidChangeActiveInstance.event;
  ...
  public async setActiveDesign(design: DesignItem | undefined) {
    ...
    // slangfixme
    if (
      design instanceof FDesignItem &&
      design.DelegateDesign instanceof SlangDesignItem
    ) {
    ...
    if (design) {
      this.treeData = design.getTreeData();
      this.activeScopeStatusBarItem.text =
        "Active scope: " + design.getActiveScope();
  ...
  getChildren(
    element?: NetlistItem | undefined,
  ): vscode.ProviderResult<NetlistItem[]> {
    if (!this.activeDesign) {
      return [];
    }
    return (
      this.activeDesign.getChildrenExternal(element) ?? Promise.resolve([])
    );
  }
  ...
  private async setContextForGoBackOrForward(
    element: NetlistItem,
    action: string,
    isGoBackOrForward: boolean,
  ) {
    if (!this.activeDesign) {
      return;
    }
    if (!isGoBackOrForward) {
      ...
      await vscode.commands.executeCommand(
        "setContext",
        "sv-pathfinder.isGoForwardEnabled",
        false,
      );
    }
  ...
  async gotoDefinition(
    element: NetlistItem,
    isGoBackOrForward: boolean = false,
  ): Promise<NetlistItem | undefined> {
    if (!this.activeDesign) {
      return;
    }
    ...
    if (element.contextValue === "instanceItem") {
      ...
      if (!scope) {
        console.log(
          "[gotoDefinition] Cannot find in hierarchy tree: " + element.fullName,
        ); // Should not happen
      } else {
        element = scope;
      }
    ...
    if (fileInfo.filePath) {
      await showTextDocumentLocation(
        fileInfo.filePath,
        fileInfo.lineNumber,
        fileInfo.columnNumber,
        this.activeDesign.isExample,
      );
    }
    ...
    await this.setContextForGoBackOrForward(
      element,
      "gotoDefinition",
      isGoBackOrForward,
    );
  ...
  public async setDriversLoadsData(element: NetlistItem) {
    this.driversTreeProvider.setDriversLoadsData(
      element.drivers,
      this.driversView,
    );
  ...
  private async setActiveInstance(element: NetlistItem) {
    ...
    this.activeScopeStatusBarItem.text =
      "Active scope: " + this.activeDesign.getActiveScope();
  ...
  async gotoInstantiation(
    ...
    if (
      element.contextValue !== "scopeItem" &&
      element.contextValue !== "instanceItem"
    ) {
      return;
    }
    ...
    if (element.contextValue === "instanceItem") {
      ...
      if (!scope) {
        console.log(
          "[gotoInstantiation] Cannot find in hierarchy tree: " +
            element.fullName,
        ); // Should not happen
  ...
  async updateWaveformValuesDescriptions(
    scope: NetlistItem,
    waveformValueMap: Map<string, string>,
  ) {
    if (scope.contextValue !== "scopeItem") {
      return;
    }
    // Assume children are already loaded
    ...
    for (const child of scope.children) {
      ...
      if (child.contextValue === "varItem" && child.type !== "parameter") {
        ...
        if (waveformValueMap.has(child.name)) {
          child.description =
            parts[0].trim() + ` = ${waveformValueMap.get(child.name)}`;
        } else {
          child.description = parts[0].trim();
        }
      }
      ...
      this.iconPath = {
        light: vscode.Uri.file(
          path.join(
            ...
            "..",
            "media",
            "file_type_light_systemverilog.svg",
          ),
        ),
        dark: vscode.Uri.file(
          path.join(__dirname, "..", "media", "file_type_systemverilog.svg"),
        ),
      };
    } else if (
      extension === ".vh" ||
...
export class DriversLoadsTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  public getTreeData(): vscode.TreeItem[] {
    return this.treeData;
  }
  ...
  public async setDriversLoadsData(
    elements: NetlistItem[],
    ...
    for (const element of elements) {
      ...
      if (!files.has(file)) {
        files.set(
          file,
          new FileItem(file, [], vscode.TreeItemCollapsibleState.Expanded),
        );
      }
...
export class ModuleInstancesTreeProvider implements vscode.TreeDataProvider<NetlistItem> {
  ...
  getChildren(
    ...
    return (
      this.activeDesign.getModuleInstancesExternal(element) ??
      Promise.resolve([])
    );
  }
