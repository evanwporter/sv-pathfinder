...
const VerilogWasmPath = path.join(
  __dirname,
...
export class Parser {
  ...
  private traverse(node: treeSitter.Node, depth: number = 0) {
    ...
    console.log(
      `[${depth}]${indent}Node type: ${node.type}, text: ${node.text}, start: ${node.startPosition.row}:${node.startPosition.column}, end: ${node.endPosition.row}:${node.endPosition.column}`,
    );
    for (const child of node.namedChildren.filter(
      (child): child is treeSitter.Node => child !== null,
    )) {
  ...
  public async parseAndCollectIdentifiersInModule(
    document: vscode.TextDocument,
    moduleName: string,
  ): Promise<treeSitter.Node[] | undefined> {
  ...
  private collectIdentifiers(
    moduleNode: treeSitter.Node | undefined,
  ): treeSitter.Node[] {
    if (!moduleNode) {
      return [];
    }
  ...
  private collectFilteredIdentifiers(
    node: treeSitter.Node,
    excludedParentTypes: string[],
  ): treeSitter.Node[] {
  ...
  private collectFilteredIdentifiersQuery(
    node: treeSitter.Node,
    excludedParentTypes: string[],
  ): treeSitter.Node[] {
    const query = new treeSitter.Query(
    ...
    const filteredIds = matches.filter((match) => {
      ...
      return (
        ...
        !(
          parent.type === "port_identifier" &&
          parent.parent &&
          parent.parent.type === "named_port_connection"
        )
      );
    });
  ...
  private collectFilteredIdentifiersCursor(
    node: treeSitter.Node,
    excludedParentTypes: string[],
  ): treeSitter.Node[] {
    ...
    while (visiting) {
      ...
      if (
        ...
        !(
          current.parent.type === "port_identifier" &&
          current.parent.parent &&
          current.parent.parent.type === "named_port_connection"
        )
      ) {
  ...
  public async getModuleAtCursor(): Promise<string | undefined> {
    ...
    for (const match of matches) {
      ...
      if (
        moduleNode &&
        moduleNode.startPosition.row <= position.line &&
        moduleNode.endPosition.row >= position.line
      ) {
