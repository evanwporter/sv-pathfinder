...
export class EditorMenuProvider {
  ...
  public async selectInstance() {
    ...
    for (const module of modules) {
      if (module.name === moduleName) {
        this.moduleInstancesView.reveal(module, {
          select: true,
          focus: false,
          expand: 3,
        });
  ...
  private async findVarItem(name: string): Promise<NetlistItem | undefined> {
    const activeInstance = this.hierarchyTreeProvider
      .getActiveDesign()
      ?.getActiveInstance();
    if (!activeInstance) {
      return;
  ...
  public async showInHierarchyView() {
    // TODO: handle module/check is variable
    const symbol = getWordAtCursor();
    if (!symbol) {
      return;
    }
    ...
    const element = await this.findVarItem(symbol);
    if (element) {
      this.hierarchyView.reveal(element, {
        select: true,
        focus: false,
        expand: 3,
      });
  ...
  private async getHierarchyName(): Promise<string | undefined> {
    ...
    const scopeName = this.hierarchyTreeProvider
      .getActiveDesign()
      ?.getActiveScope();
